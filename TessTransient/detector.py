...
class TessDetector():
    ...
    def detect(self,significance,min_length,min_px,max_events,plot,asteroids):
        ...
        self.order = _Rank_brightness(self.flux,events,eventtimes,eventmask)     
        self.events = events[self.order]
        self.eventtimes = eventtimes[self.order]
        self.eventmask = eventmask[self.order]
    ...
    def event_cutout(self,eventnumber,size=50,phot=None):
        """
        Printout PS1 or SkyMapper cutouts of event region.
        ...
        ------
        phot : str
            specify between 'PS1' or 'SkyMapper' 
        """

        _,coords = self._find_coords(eventnumber)
